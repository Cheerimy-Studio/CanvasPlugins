...
public class Finder extends SimpleFileVisitor<Path> {
    ...
    public void fileWalk(String datapacksFolderPath, File file, boolean isZip) throws IOException {
        ...
        if (isZip) {
            // 防 Zip Slip：所有解压条目必须落在 targetFile 目录内
            Path targetRoot = targetFile.getCanonicalFile().toPath();
            try (ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(file.toPath()))) {
                ...
                while (zipEntry != null) {
                    ...
                    if (!childFile.getCanonicalFile().toPath().startsWith(targetRoot)) {
                        throw new IOException(
                                "Zip entry escapes target directory (possible Zip Slip): " + zipEntry.getName());
                    }
